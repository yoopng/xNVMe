...
xnvme_be_linux_ucmd_iov(struct xnvme_cmd_ctx *ctx, struct iovec *dvec, size_t dvec_cnt,
			size_t dvec_nbytes, struct iovec *mvec,
			size_t mvec_cnt, size_t mvec_nbytes)
{
	...
	if (mvec || mvec_cnt || mvec_nbytes) {
		XNVME_DEBUG("FAILED: mvec or mvec_nbytes provided");
		return -ENOSYS;
	}
	...
	ctx->cmd.passthru.mptr = (uint64_t)mvec;
	ctx->cmd.passthru.metadata_len = mvec_cnt;
