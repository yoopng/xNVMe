			     ...
			     void *XNVME_UNUSED(mbuf), size_t XNVME_UNUSED(mbuf_nbytes))
{
	...
	case XNVME_SPEC_NVM_OPC_WRITE:
		memcpy((char *)(state->ramdisk) + (size_t)(ctx->cmd.nvm.slba << ssw), dbuf,
		       dbuf_nbytes);
		break;

	case XNVME_SPEC_NVM_OPC_READ:
		memcpy(dbuf, (char *)(state->ramdisk) + (size_t)(ctx->cmd.nvm.slba << ssw),
		       dbuf_nbytes);
		break;

	case XNVME_SPEC_FS_OPC_WRITE:
		memcpy((char *)(state->ramdisk) + (size_t)(ctx->cmd.nvm.slba), dbuf, dbuf_nbytes);
		break;

	case XNVME_SPEC_FS_OPC_READ:
		memcpy(dbuf, (char *)(state->ramdisk) + (size_t)(ctx->cmd.nvm.slba), dbuf_nbytes);
		break;
