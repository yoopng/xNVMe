...
{
	struct xnvme_opts opts = { .create = 1, .wronly = 1, .direct = cli->args.direct, .be = cli->args.be, .sync = cli->args.sync };
	struct xnvme_dev *fh;
	struct xnvme_cmd_ctx ctx;
	const char *fpath;
...
{
	struct xnvme_opts opts = { .create = 1, .wronly = 1, .direct = cli->args.direct, .be = cli->args.be, .async = cli->args.async };
	struct xnvme_dev *fh;
	struct xnvme_queue *queue = NULL;
	struct cb_args cb_args = { 0 };
	const char *fpath;
	...
	xnvmec_pinf("dump-async-iovec{fpath: %s, tbytes: %zu, buf_nbytes: %zu, iosize: %zu, qdepth: %d}",
		    fpath, tbytes, buf_nbytes, iosize, qdepth);
...
static struct xnvmec_sub g_subs[] = {
	...
	},
	{
		...
		"Write a buffer of 'data-nbytes' to file", dump_sync_iovec, {
			{XNVMEC_OPT_DATA_OUTPUT, XNVMEC_POSA},
			{XNVMEC_OPT_DATA_NBYTES, XNVMEC_LREQ},
			{XNVMEC_OPT_IOSIZE, XNVMEC_LOPT},
			{XNVMEC_OPT_DIRECT, XNVMEC_LFLG},
	...
	},
	{
		...
		"Write a buffer of 'data-nbytes' to file --data-output", dump_async_iovec, {
			{XNVMEC_OPT_DATA_OUTPUT, XNVMEC_POSA},
			{XNVMEC_OPT_DATA_NBYTES, XNVMEC_LREQ},
			{XNVMEC_OPT_IOSIZE, XNVMEC_LOPT},
			{XNVMEC_OPT_QDEPTH, XNVMEC_LOPT},
